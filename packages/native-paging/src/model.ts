...
export class PageResponse extends Schema.Struct({
  items: Schema.Array(
    Schema.Struct({
...
export const connectPager = <E, R>({
  pager,
  load,
}: {
  readonly pager: Pick<NativePager, 'addListener' | 'start' | 'resolve' | 'reject' | 'close'>;
  readonly load: (
    request: Pick<typeof PageRequest.Type, 'offset' | 'limit'>
  ) => Effect.Effect<typeof PageResponse.Type, E, R>;
}) =>
  Effect.gen(function* () {
    ...
    const requests = pager.addListener('request', (request) => {
      ...
      run(
        ...
        Effect.gen(function* () {
          ...
          const page = yield* Schema.decodeEffect(PageResponse)(response);
          if (active) {
