...
export type TursoSyncClientOptions<R = never> = CoreTursoSyncClientOptions<R> &
  Omit<DatabaseOpts, keyof CoreTursoSyncClientOptions<R>>;

/**
 * Creates a scoped Effect SQL client backed by one serialized React Native
 * Turso connection.
 */
export const make = Effect.fnUntraced(function* <R = never>(options: TursoSyncClientOptions<R>) {
  const { connect } = yield* Effect.promise(async () => import('@tursodatabase/sync-react-native'));
