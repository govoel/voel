...
class VoelAudioPlayer(
    ...
    private fun createMediaItem(source: AudioSource): MediaItem {
        val sourceUri =
                if (Util.isLocalFileUri(source.fileUri.toUri())) {
                    Uri.fromFile(File(source.fileUri))
                } else {
                    source.fileUri.toUri()
                }
    ...
    fun currentStatus(): Map<String, Any?> {
        ...
        return mapOf(
                ...
                "errorCode" to controller.getPlayerError()?.let { it.errorCode }
        )
    }
}
